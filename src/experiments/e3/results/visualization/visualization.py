...
for i, num in enumerate(all_odd_composite_numbers):
    ...
    if times_values[i]:
        plt.scatter([num] * len(times_values[i]), times_values[i], color='purple', alpha=0.5, label=f'Our Implementation Not Optimized {num}' if i == 0 else "")
  
# Plot average and mean trend lines with interpolation (skip missing values)
plt.plot(all_odd_composite_numbers, avg, color='purple', label='Avg Our Implementation Not Optimized', linestyle='-', marker='o', markersize=5)
plt.plot(all_odd_composite_numbers, mean, color='purple', linestyle='--', label='Mean Our Implementation Not Optimized', marker='x', markersize=5)
