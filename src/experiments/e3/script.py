...
def run(run_shor, TimeTracker):
    ...
    with open("./e3/results/times.json", "w") as f:
...
def get_data(path='./e3/data.json'):
    try:
...
def run_set(run_shor, category_name, data_set, tracker, results):
    for i in data_set:
        ...
        for j in range(5):
            ...
            run_shor(i, isSimulator=True, isCircuitOptimized=False, isPrinting=False)
            tracker.stop()
...
def write_result(number, category_name, time):
    file_path = f"./e3/results/times.json"
...
def analyze():
    # Read the input JSON file
    with open("./e3/results/times.json", "r") as file:
        data = json.load(file)
    ...
    with open("./e3/results/average_mean.json", "w") as outfile:
