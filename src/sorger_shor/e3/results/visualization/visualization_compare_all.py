...
# Load all data files
avg_mean_not_optimized = load_json('../average_mean_no_lucky_guesses.json')
avg_mean_optimized = load_json('../../../e1/results/average_mean_no_lucky_guesses.json')
avg_mean_normal = load_json('../../../e2/results/average_mean_normal.json')
avg_mean_sequential = load_json('../../../e2/results/average_mean_sequential.json')


times_not_optimized = load_json('../times_no_lucky_guesses.json')
times_optimized = load_json('../../../e1/results/times_no_lucky_guesses.json')
times_normal = load_json('../../../e2/results/times_normal.json')
times_sequential = load_json('../../../e2/results/times_sequential.json')

# Extract the composite numbers from all files
odd_composite_numbers_not_optimized = set(map(int, avg_mean_not_optimized['odd_composite_numbers'].keys()))
...
odd_composite_numbers_times_not_optimized = set(map(int, times_not_optimized['odd_composite_numbers'].keys()))
...
all_odd_composite_numbers = sorted(odd_composite_numbers_normal | odd_composite_numbers_optimized | odd_composite_numbers_times_not_optimized | odd_composite_numbers_times_optimized | odd_composite_numbers_not_optimized | odd_composite_numbers_times_normal | odd_composite_numbers_sequential | odd_composite_numbers_times_sequential)

# Prepare data for plotting (handle missing values as NaN)
avg_not_optimized = [avg_mean_not_optimized['odd_composite_numbers'].get(str(num), [nan, nan])[0] for num in all_odd_composite_numbers]
mean_not_optimized = [avg_mean_not_optimized['odd_composite_numbers'].get(str(num), [nan, nan])[1] for num in all_odd_composite_numbers]
...
times_not_optimized_values = [times_not_optimized['odd_composite_numbers'].get(str(num), []) for num in all_odd_composite_numbers]
...
for i, num in enumerate(all_odd_composite_numbers):
    ...
    if times_not_optimized_values[i]:
        plt.scatter([num] * len(times_not_optimized_values[i]), times_not_optimized_values[i], color='purple', alpha=0.5, label=f'Our Implementation Not Optimized No Lucky Guesses {num}' if i == 0 else "")
    # Scatter for Optimized mode
    # Scatter for Normal mode
    if times_normal_values[i]:
        plt.scatter([num] * len(times_normal_values[i]), times_normal_values[i], color='blue', alpha=0.5, label=f'Normal QFT {num}' if i == 0 else "")
...
plt.plot(all_odd_composite_numbers, avg_not_optimized, color='purple', label='Avg Our Implementation Not Optimized No Lucky Guesses', linestyle='-', marker='o', markersize=5)
plt.plot(all_odd_composite_numbers, avg_normal, color='blue', label='Avg Normal QFT', linestyle='-', marker='o', markersize=5)
...
plt.plot(all_odd_composite_numbers, mean_not_optimized, color='purple', linestyle='--', label='Mean Our Implementation Not Optimized No Lucky Guesses', marker='x', markersize=5)
plt.plot(all_odd_composite_numbers, mean_normal, color='blue', linestyle='--', label='Mean Normal QFT', marker='x', markersize=5)
...
all_times = [time for sublist in times_normal_values + times_not_optimized_values + times_optimized_values + times_normal_values + times_sequential_values for time in sublist if time is not None]
