...
# Load all data files
avg_mean_normal = load_json('../average_mean_normal.json')
avg_mean_sequential = load_json('../average_mean_sequential.json')
avg_mean = load_json('../../../e1/results/average_mean_no_lucky_guesses.json')
times_normal = load_json('../times_normal.json')
times_sequential = load_json('../times_sequential.json')
times = load_json('../../../e1/results/times_no_lucky_guesses.json')
...
all_odd_composite_numbers = sorted(odd_composite_numbers_normal | odd_composite_numbers_sequential | odd_composite_numbers_times_normal | odd_composite_numbers_times_sequential | odd_composite_numbers | odd_composite_numbers_times)
...
for i, num in enumerate(all_odd_composite_numbers):
    ...
    if times_values[i]:
        plt.scatter([num] * len(times_values[i]), times_values[i], color='green', alpha=0.5, label=f'Our Implementation Optimized No Lucky Guesses {num}' if i == 0 else "")
...
plt.plot(all_odd_composite_numbers, avg, color='green', label='Avg Our Implementation Optimized No Lucky Guesses', linestyle='-', marker='o', markersize=5)
...
plt.plot(all_odd_composite_numbers, mean, color='green', linestyle='--', label='Mean Our Implementation Optimized No Lucky Guesses', marker='x', markersize=5)

# Set dynamic y-axis limits
all_times = [time for sublist in times_normal_values + times_sequential_values for time in sublist if time is not None]
